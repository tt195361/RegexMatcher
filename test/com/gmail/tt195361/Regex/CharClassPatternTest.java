...
public class CharClassPatternTest {
	...
	private void checkOneMatch(
		...
		CharClassPattern charClassPat = new CharClassPattern(notContains, charSet);
		RegexPatternTest.checkOneMatch(
				charClassPat, str, expectedResult, expectedCurrentIndex, message);
	}
	
	static void check(
			CharClassPattern expected, CharClassPattern actual, String message) {
		assertEquals(
				"notContains: " + message,
